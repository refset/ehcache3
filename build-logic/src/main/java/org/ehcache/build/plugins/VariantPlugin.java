    ...
    project.getPlugins().withType(JavaPlugin.class).configureEach(javaPlugin ->  {
      ...
      variants.getVariants().configureEach(variant -> {
        if (variant.hasSources().get()) {
          ...
          Configuration commonApi = bucket(project, API_CONFIGURATION_NAME, COMMON_SOURCE_SET_NAME);
          project.getConfigurations().named(mainSource.getApiConfigurationName()).configure(config -> config.extendsFrom(commonApi));
          Configuration commonCompileOnlyApi = bucket(project, COMPILE_ONLY_API_CONFIGURATION_NAME, COMMON_SOURCE_SET_NAME);
          ...
          Configuration commonImplementation = bucket(project, IMPLEMENTATION_CONFIGURATION_NAME, COMMON_SOURCE_SET_NAME);
          ...
          Configuration commonRuntimeOnly = bucket(project, RUNTIME_ONLY_CONFIGURATION_NAME, COMMON_SOURCE_SET_NAME);
          ...
          Configuration api = bucket(project, API_CONFIGURATION_NAME, variant.getName()).extendsFrom(commonApi);
          Configuration implementation = bucket(project, IMPLEMENTATION_CONFIGURATION_NAME, variant.getName()).extendsFrom(api, commonImplementation);
          Configuration compileOnlyApi = bucket(project, COMPILE_ONLY_API_CONFIGURATION_NAME, variant.getName()).extendsFrom(commonCompileOnlyApi);
          Configuration runtimeOnly = bucket(project, RUNTIME_ONLY_CONFIGURATION_NAME, variant.getName()).extendsFrom(commonRuntimeOnly);
