...
public class PackagePlugin implements Plugin<Project> {
  ...
  private void createPackage(Project project, String variant, List<Capability> capabilities) {
    ...
    Configuration implementation = bucket(project, camelPrefix(variant, IMPLEMENTATION_CONFIGURATION_NAME)).extendsFrom(api);
    Configuration compileOnlyApi = bucket(project, camelPrefix(variant, COMPILE_ONLY_API_CONFIGURATION_NAME));
    Configuration runtimeOnly = bucket(project, camelPrefix(variant, RUNTIME_ONLY_CONFIGURATION_NAME));
